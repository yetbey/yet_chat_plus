...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // İstek gövdesinden post_id ve content'i al
    const { post_id, content } = await req.json()
    if (!post_id || !content || typeof content !== 'string' || content.trim().length === 0) {
      return new Response(JSON.stringify({ error: 'post_id ve geçerli bir content gerekli' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400,
      })
    }
    ...
    const { data, error } = await supabaseAdmin.rpc('add_post_comment', {
      p_post_id: post_id,
    ...
    // SQL fonksiyonundan dönen yeni yorum verisini istemciye gönder
